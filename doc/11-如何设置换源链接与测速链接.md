...
在 `_prepare()` 函数中，在 `def_sources_begin()` 的位置的**第二列**，新增/修改你想要替换的链接。
...
{&某新镜像站,       "某镜像站提供的换源链接"，                         FeedByPrepare}
def_sources_end()
...
在 `_prepare()` 函数中，在 `def_sources_begin()` 的位置的**第三列**，设置/修改你想要替换的链接。
...
3. 在下面代码的第四行中，我们设置了一个宏 `FeedByPrepare`，这意味着我们将在 `_prepare()` 函数的后续动态计算 URL
